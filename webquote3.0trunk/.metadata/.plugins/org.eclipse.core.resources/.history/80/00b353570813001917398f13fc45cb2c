...
//			}
	...
	public String addNewUser() {
		...
		lstBizUnits=UserInfo.getUser().getAdminBizUnits();
		roles = new DualListModel<Role>(roleSB.findAllForUserAdmin(), new ArrayList<Role>());

		bizUnits = new DualListModel<BizUnit>(UserInfo.getUser().getAdminBizUnits(), new ArrayList<BizUnit>());
...
//		filteredDataAccessVos = dataAccessVos;
	...
	public void copyUser() {
		...
		if(copiedUser.isSalsCostAccessFlag()==null){
			user.setSalsCostAccessFlag(null);
		}else if(!copiedUser.isSalsCostAccessFlag()){
			salsCostAccessStr ="No";
			user.setSalsCostAccessFlag(false);
		}else{
			salsCostAccessStr ="Yes";
			user.setSalsCostAccessFlag(true);
		}
