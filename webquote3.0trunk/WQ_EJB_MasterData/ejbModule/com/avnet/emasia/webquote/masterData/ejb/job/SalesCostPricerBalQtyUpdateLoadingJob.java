...
@Stateless
@Remote(SalesCostPricerBalQtyUpdateRemote.class)
public class SalesCostPricerBalQtyUpdateLoadingJob extends StandardJob implements SalesCostPricerBalQtyUpdateRemote
{
	private static Logger logger = Logger.getLogger(SalesCostPricerBalQtyUpdateLoadingJob.class.getName());
	...
	public void executeTask(Timer timer) {
		try {
			...
			logger.log(Level.SEVERE, "Exception occured for SalesCostPricerBalQtyUpdateLoadingJob#Timer: " + timer.getInfo() + ", Task: " + this.getType()
	...
	// For control-M
	/* (non-Javadoc)
	 * @see com.avnet.emasia.webquote.masterData.ejb.remote.SalesCostPricerBalQtyUpdateRemote#executeTask()
	 */
	@Override
	...
	public void saveToDB(FileContext fileCon) throws CheckedException {
		...
		logger.info("SalesCostPricerBalQtyUpdateLoadingJob.saveToDB : BEGIN");
...
//							fileCon.getDataFiles()[j][i].getFile().getName());
		...
		logger.info("SalesCostPricerBalQtyUpdateLoadingJob.saveToDB : END");
	}
...
//	@Override
	public void transferToDB(List<String> dataList, Config config, String functionName, String fileName)
		...
		logger.info("SalesCostPricerBalQtyUpdateLoadingJob.transferToDB : BEGIN {" + functionName + "," + fileName + "}");
		//List<Object> returnList = new ArrayList<Object>();
		//List<String> fields = FunctionUtils.splitLineData((String) dataList.remove(0), Constants.SEPARATOR, true);
		int size = dataList.size();
		List<String> rowData = null;
		
		for (int i = 1; i < size; i++) {	//start from 1, skip the header (or need to call dataList.remove(0))
			try {
...
//				logger.info("SalesCostPricerBalQtyUpdateLoadingJob.transferToDB : DataLine {" + dataList.get(i) + "} " + rowData.get(10));
		...
		logger.info("SalesCostPricerBalQtyUpdateLoadingJob.transferToDB : END");
		//return returnList;
	}
