...
public class PricerDeletedDAOImpl implements PricerDeletedDAO {
	...
	public void batchStartUpdate() throws IllegalStateException, SecurityException, SystemException {
		...
		try {
			...
			if (prcID>0) sJPQL = "UPDATE PricerDeleted v set v.deletedBatchStatus = v.deletedBatchStatus + 10 where v.deletedBatchStatus >= 0 AND v.deletedBatchStatus < 10 AND (SELECT COUNT(o) FROM v.material.sapMaterials o)> 0 AND v.deletedId<" + prcID;
			else sJPQL = "UPDATE PricerDeleted v set v.deletedBatchStatus = v.deletedBatchStatus + 10 where v.deletedBatchStatus >= 0 AND v.deletedBatchStatus < 10 AND (SELECT COUNT(o) FROM v.material.sapMaterials o)> 0";
