...
public class DateUtils {
	...
	public static String getDateByPattern(String prm_pattern) {
		SimpleDateFormat formatter = new SimpleDateFormat(prm_pattern);
		return formatter.format(new java.util.Date());
	}

	public static Date switchStringToDate(String prm_Date, String prm_pattern) {
		SimpleDateFormat formatter = new SimpleDateFormat(prm_pattern);
		try {
			return formatter.parse(prm_Date);
		} catch (ParseException e) {
			return null;
		}
	}

	public static String formateDate2String(Date date, String prm_pattern) {
		SimpleDateFormat formatter = new SimpleDateFormat(prm_pattern);
		return formatter.format(date);
	}

	// input "yyyyMMdd" output "yyyyMMdd"
	public static String afterDate(String prm_date, int prm_iAfterDay) {
		Calendar cal = Calendar.getInstance();
		cal.set(Integer.parseInt(prm_date.substring(0, 4)), Integer
				.parseInt(prm_date.substring(4, 6)) - 1, Integer
				.parseInt(prm_date.substring(6, 8)));
		cal.add(Calendar.DATE, prm_iAfterDay);
