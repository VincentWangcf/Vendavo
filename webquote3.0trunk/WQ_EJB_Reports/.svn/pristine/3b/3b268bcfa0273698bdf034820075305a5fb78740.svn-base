...
public class WorkingPlatFormOffLineReport extends OffLineReport  implements OffLineRemote {
	...
	public void sendOffLineReport(OfflineReportParam param) throws Exception {
		...
		try {
			...
			int totalRecords = quoteSB.findLazyDataCount(criteria, null, null, null, param.getFiltersMap());
			...
			try { 
	           ...
	           if("cis2115vmts".equalsIgnoreCase(address)) { 
	        	   ...
	        	   reportFilePath = "C:/temp/offlinefiles";
	           }
	        } catch (Exception e) { 
	            e.printStackTrace();
	        } 
			//reportFilePath = "C:"+File.separator+"template0312";
			//templatePath =  "C:"+File.separator+"template0312";
			...
			for (int i = 0; i <= count; i++) {
				...
				List<QuoteItem> quoteItems = quoteSB.findLazyData(criteria, first, end, null, "ASC", param.getFiltersMap());
			...
			workbook.dispose();
			LOG.log(Level.SEVERE, "Exception WorkingPlatForm sendOffLineReport : "+e.getMessage(), e);
			throw e;
		}finally {
			if(resultList != null) {
				resultList.clear();
