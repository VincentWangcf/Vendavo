...
public class CatalogSearchBuilderProcessSheet implements ProcessSheetInterface {
	private List<CatalogSearchItemBean> beans = new ArrayList<CatalogSearchItemBean>();
	private List<ExcelAliasBean> excelAliasTreeSet = POIUtils.getQuoteBuilderExcelAliasAnnotation(CatalogSearchItemBean.class);
	private CatalogSearchItemBean bean = null;
	...
	public void rowCompleted() {
		if (currentRow >= 0 ) {
			...
			bean = new CatalogSearchItemBean();
			bean.setLineSeq(currentRow + 2);
		}
	}
	
	@SuppressWarnings({ "unchecked", "rawtypes" })
	private boolean isNullBean(CatalogSearchItemBean bean) {
		boolean isNullBean = true;
		try {
			if (bean != null) {
				List<ExcelAliasBean> excelAliasTreeSet = POIUtils
						.getQuoteBuilderExcelAliasAnnotation(CatalogSearchItemBean.class);
