...
public class CommodityMaintainMB extends CommonCommodity  implements java.io.Serializable{
    ...
    private List<CatalogMaintVo> catalogMaintVos;
    ...
    public LazyDataModel<CatalogMaintVo> catalogMaintLazyModel;
...
//    		logger.fine("initialize checkBoxSet is null");
	 ...
	 */
	public void searchQtyBreakPricer()
	{
		...
		if (d != null) {
		    ...
		    pisGolbal = pis;
		    this.catalogMaintLazyModel = new LazyDataModel<CatalogMaintVo>()
	        {
	            private static final long    serialVersionUID    = 1L;
	            private List<CatalogMaintVo> data;
	            @Override
	            public List<CatalogMaintVo> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,Object> filters) 
	            {
	               ...
	               // programMaterialSB.searchPricerByPage(pisGolbal);
	                catalogMaintVos = TransferHelper.transferPricerToPIMainVo(result,bizUnitStr);
	                this.data = catalogMaintVos;
	                //programMaterialLazyModel.setRowCount(newResult.size());
	                int rowCount = programMaterialSB.searchQtyBreakPricerCount(pisGolbal);
	                catalogMaintLazyModel.setRowCount(rowCount);
	                return catalogMaintVos;
	            }
	            @Override
	            public CatalogMaintVo getRowData(String rowKey) 
	            {
	                if(this.data!=null && this.data.size()>0)
	                for(CatalogMaintVo pm : this.data) 
	            ...
	            @Override
	            public Object getRowKey(CatalogMaintVo pm) {
	                return pm.getId();
	            }
	        };
	...
	public List<CatalogMaintVo> getCatalogMaintVos() {
		return catalogMaintVos;
	}

	public void setCatalogMaintVos(
			List<CatalogMaintVo> programMaterialMaintVos) {
		this.catalogMaintVos = programMaterialMaintVos;
	}
	...
	public void setBizUnit(BizUnit bizUnit) {
	 ...
	 public void switchBizUnit(){
		...
		public LazyDataModel<CatalogMaintVo> getCatalogMaintLazyModel()
		{
			return catalogMaintLazyModel;
		}

		public void setCatalogMaintLazyModel(
				LazyDataModel<CatalogMaintVo> programMaterialLazyModel)
		{
			this.catalogMaintLazyModel = programMaterialLazyModel;
		}
