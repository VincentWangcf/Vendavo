...
abstract public class SelectableLazyDataModel<T> extends LazyDataModel<T> implements SelectableDataModel<T>, Serializable {
	...
	public synchronized List<T> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,Object> filters) {
		...
		if(outcome == null && map.size() ==0) {
			...
			int f = first;
			futResultItems = exc.submit(()-> findLazyData(f, pageSize, sortField, sort, filters));
		}
		...
		if(map.size() == 0){
			...
			LOGGER.info("COUNTS take time " +(System.currentTimeMillis()-c));
			if(count == 0){
				count = -1;
			}
	...
	public void onToggleSelect(ToggleSelectEvent event) {
		...
		LOGGER.log(Level.INFO, "call onToggleSelect " + selectionItems.size() + " cache " + this.getCacheSelectedItems().size());
	}
...
//		selectionDatas.add(t);
		LOGGER.log(Level.INFO, "call onRowSelectCheckbox " + selectionItems.size() + " cache " + this.getCacheSelectedItems().size());
	}
...
//		selectionDatas.remove(t);
		LOGGER.log(Level.INFO, "call onRowUnselectCheckbox " + selectionItems.size() + " cache " + this.getCacheSelectedItems().size());
	}
	...
	public void onRowSelect(SelectEvent event) {
		...
		this.getCacheSelectedItems().removeAll(getCurrentPageItems());
		this.getCacheSelectedItems().addAll(selectionItems);
		LOGGER.log(Level.INFO, "call onRowSelect " + selectionItems.size() + " cache " + this.getCacheSelectedItems().size());
	}

	public void onRowUnselect(UnselectEvent event) {
		T t = (T) event.getObject();
		this.getCacheSelectedItems().remove(t);
		LOGGER.log(Level.INFO, "call onRowUnselect " + selectionItems.size() + " cache " + this.getCacheSelectedItems().size());
	}
