...
public class RefreshQuoteMB extends AttachmentEditMB implements Serializable {
	...
	public SelectItem[] convertCurrencyToSelectItem(QuoteItem quoteItem){
		...
		if(quoteItem != null && quoteItem.getQuote() != null && quoteItem.getQuote().getBizUnit() != null)
		{
			...
			if(defaultCurrency != null && quoteItem.getRfqCurr() == null)
				quoteItem.setRfqCurr(Currency.getByCurrencyName(defaultCurrency.getCurrency()));
	...
	public void checkRrefreshQuoteItem(List<QuoteItemVo> quoteItemVos){		
		...
		for(QuoteItemVo vo : quoteItemVos){
			...
			if (item.getQuotedPrice()!=null) {
				BigDecimal qPrice = item.convertToRfqValueWithDouble(item.getQuotedPrice());
				
				if (qPrice!=null) item.setTargetResale(qPrice.doubleValue());
				else item.setTargetResale(null);
			}
			else
				item.setTargetResale(null);
			...
			if(StringUtils.isEmpty(item.getRfqCurr().name())){
				ExcCurrencyDefault ecd = exchangeRateSB.findDefaultCurrencyByBu(item.getQuote().getBizUnit().getName());
				if(ecd != null){
					vo.getQuoteItem().setBuyCurr(Currency.USD);
					vo.getQuoteItem().setRfqCurr(Currency.getByCurrencyName(ecd.getCurrency()));
				}
			}			
		}
		checkDrmsLinkage();
	...
	private List<String> validation2(){	
		...
		for(QuoteItemVo vo : quoteItemVos){
			try{
				...
				LOG.log(Level.INFO, "begin to validate quote item:"+item.getQuoteNumber());
	...
	private boolean targetPriceChecking(QuoteItem item) {
		...
		Double price; // = item.convertToRfqValueWithDouble(item.getQuotedPrice()).doubleValue();
		...
		} else price = item.convertToRfqValueWithDouble(item.getQuotedPrice()).doubleValue();
	...
	private boolean pmoqChecking(QuoteItem item){
		...
		if("Exact".equalsIgnoreCase(qtyIndicator)){
