...
//	    document.getElementById("form_pendinglist:sqCount").innerHTML = 'SQ : ' + args.sqCount;
		...
		if(mfrCode_size > 0){
			...
			/*alert(eval('args.mfrCode'+0));
			alert(eval('args.mfrCode'+1));
			alert(mfrCode_size);*/
		...
		if(bmt_size > 0){
			var bmts = document.getElementById("form_pendinglist:datatable_pendinglist:p_bmt:filter");
			var selectedBmt = '';
			for(i = 0; i < bmts.options.length; i++){
				if(bmts.options[i].selected == true){
					selectedBmt = bmts.options[i].value;
					break;
				}
			}						
			bmts.options.length = 0;
			bmts.options[0] = new Option('All', '');    	
			for(i = 0; i < bmt_size; i++){
				var bmt_k = eval('args.bmtKey'+i);
				
				bmt_k = unescape(bmt_k);			
				var bmt_v = eval('args.bmtValue'+i);
				bmt_v = unescape(bmt_v);			
				bmts.options[i+1] = new Option(bmt_v, bmt_k); 
				if(bmts.options[i+1].value == selectedBmt)
					bmts.options[i+1].selected = true;
			}
		}
...
function handleFilteredLazy(args){
	...
	if(args){    
		...
		var soldTo_size = args.soldTo_size;
		var bmt_size = args.bmt_size;
		var dRegion_size = args.dRegion_size;
		var i=0;
		if(mfrCode_size > 0){
			var mfrCodes = document.getElementById("form_pendinglist:datatable_pendinglist:p_mfr:filter");
			var selectedMfr = '';
		...
		if(dRegion_size > 0){
			var ds = document.getElementById("form_pendinglist:datatable_pendinglist:p_dregion:filter");
			var sDRegion = '';
			for(i = 0; i < ds.options.length; i++){
				if(ds.options[i].selected == true){
					sDRegion = ds.options[i].value;
				}
			}						
			ds.options.length = 0;
			//"", "-select-"
			ds.options[0] = new Option('-select-', '');    	
			for(i = 0; i < dRegion_size; i++){
				var r_v = eval('args.dRegion'+i);
				r_v = unescape(r_v);
				if(r_v != '') {
					r_v = r_v.decodeHTML();
				}
				ds.options[i+1] = new Option(r_v, r_v); 
				if(ds.options[i+1].value == sDRegion)
					ds.options[i+1].selected = true;
			}
		}
	}
} 
...
/* decodeHtml
function decodeSpcialCharacter(text) {
...
function checkUpdateDetailPanel(editmode, changed, currentNumber, rowInd){
	if(currentNumber != previousQuoteItemId){
		PF('datatable_pendinglist_var').unselectAllRows();
		PF('datatable_pendinglist_var').selectRow(rowInd);
