...
public class CacheUtil {
	...
	public void init(){
		...
		.setReadThrough(true).setStoreByValue(false)
...
//				new TreeMapCacheLoader(em)));
	...
	public void initSystemConfig(){
		...
		try{
			...
			appProperty.setPropValue("true");
			sysConfigSB.updateProperty(appProperty);
			LOG.info("initSystemConfig...inner try cath");
	...
	private int getCacheExpiryTime(){
		...
		LOG.info("DEFAULT Expiry time: " + Constants.EXPIRY_TIME);
...
//    	designLocation.add("-select-"); 
	...
	public List<PageComponentItem> getPageComponents() {
		List<PageComponentItem> r = em
				.createQuery("select c from PageComponentItem c ")
				.getResultList();
		/*LOG.config(()->"getPageComponents():::" + r );
		r.stream().forEach( p ->
			LOG.config(() ->"::::" + p.getLogicCalConfig())
		);*/
		return r;
	...
	public void clear(ActionIndicator actionIndicator) {
		LOG.info("Clear this WQ mapCache :::::::::" + System.identityHashCode(this.mapCache));
		switch (actionIndicator) { 
		case CLEARALL :
			LOG.info("Clear All Begin.");
	    	this.mapCache.clear();
	    	LOG.info("Clear All end.");
			break;
		case CLEAREXRATES:
			LOG.info("Clear ExRates Begin.");
	    	//this.mapCache.remove(Constants.EXCHANGE_RATE);
	    	LOG.info("Clear ExRates end.");
			break;
		default :
			LOG.warning(" No Enum match " + actionIndicator);
			break;
		}
	}
}
