...
public class ShopCartSubmitResultDownLoadStrategy extends DownLoadStrategy {
	...
	private static final long serialVersionUID = 18268106349214389L;
	
	private  boolean qco ;
	public ShopCartSubmitResultDownLoadStrategy(boolean isQco)
	{
		this.qco = isQco;
	}
	/*
	...
	public void setDateTosheet(XSSFWorkbook wb, List<?> data) {
		...
		if (data != null) {
			for (int j = 0; j < data.size(); j++) {
				...
				//df no display
				if (obj.getQuote().getFormNumber() != null && !(obj.getQuote().getFormNumber().startsWith("DF"))) {
				...
				if (obj.getQuoteNumber() != null 
						&&(com.avnet.emasia.webquote.utilities.common.Constants.RFQ_STATUS_AQ.equalsIgnoreCase(obj.getStatus()) || this.qco)) {
				...
				if (obj.getRfqCurr()!= null) {
					cell.setCellValue(obj.getRfqCurr().name());
				}
