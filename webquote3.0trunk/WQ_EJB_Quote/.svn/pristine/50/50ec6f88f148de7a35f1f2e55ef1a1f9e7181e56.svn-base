	...
	private void updateQuoteItem(DMesgApprove dMesg) {
		...
		if(!items.isEmpty()) {
			for(QuoteItem item :items) {
				...
				// add by Vincent 20181023
				if(dMesg.getOfferPricerCurr() !=null) {
					item.setBuyCurr(Currency.valueOf(dMesg.getOfferPricerCurr()));
				}
	...
	private void SendException(ExceptionVo exceptionVo, File file) {
		...
		try {
			if(null!=exceptionVo) {
				...
				String emailStr = sysConfigSB.getProperyValue(TIEntityFactory.TI_RECIEVER_EMAILS);
				if(emailStr == null) {
