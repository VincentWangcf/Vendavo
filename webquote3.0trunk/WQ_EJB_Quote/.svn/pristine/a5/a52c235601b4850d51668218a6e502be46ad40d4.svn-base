...
public class WorkingPlatformItemVO implements Serializable {
    ...
    private String exRateBuyStr;
    
    private String exRateRfqStr;
	...
	public DecimalFormat df2_5 = new DecimalFormat("#");
	...
	public void setQuoteItem(QuoteItem quoteItem, boolean updateQuotedMaterial, BizUnit bizUnit) {
		...
		BigDecimal tResaleBuy = toBuyCurrValue(this.quoteItem.getTargetResale());
		...
		/*resaleMarginStr = null;
		BigDecimal qtedPriceBuy = toBuyCurrValue(this.quoteItem.getQuotedPrice());
		if (this.quoteItem.getCost() != null && qtedPriceBuy != null && qtedPriceBuy.doubleValue() > 0.0) {
			Double resaleMargin = 100 * (qtedPriceBuy.doubleValue() - this.quoteItem.getCost()) / qtedPriceBuy.doubleValue();
			resaleMarginStr = df2.format(resaleMargin) + "%";
		}*/
		...
		if (this.quoteItem.getQuote() != null && this.quoteItem.getQuote().isdLinkFlag()) {
			dollarLinkStr = "Yes";
		} else {
			dollarLinkStr = "No";
		}
		
		if(this.quoteItem.getRfqCurr() != null) {
			this.rfqCurrStr = this.quoteItem.getRfqCurr().toString();
		}
		
		if(this.quoteItem.getBuyCurr() != null) {
			this.buyCurrStr = this.quoteItem.getBuyCurr().toString();
		}
		//this.exRateRfqStr  = this.quoteItem.getExRateBuy();
		refreshInfoByBuyCurr();
		
		
		
	}
	
	public void refreshInfoByBuyCurr() {
		this.targetResaleStr_buy = convertAsBuyCurrAmtStr(quoteItem.getTargetResale());
		quotedPriceStr_rfq = convertAsRfqCurrAmtStr(quoteItem.getQuotedPrice());
		this.exRateStr = calExRate();
		targetMarginStr = null;
		BigDecimal tResaleBuy = quoteItem.convertToBuyValueWithDouble(this.quoteItem.getTargetResale());
		if (this.quoteItem.getCost() != null && tResaleBuy != null && tResaleBuy.doubleValue() > 0.0) {
			Double targetMargin = 100 * (tResaleBuy.doubleValue() - this.quoteItem.getCost()) / tResaleBuy.doubleValue();
			targetMarginStr = df2.format(targetMargin) + "%";
		}
	}
	
	private String convertAsBuyCurrAmtStr(Double value) {
		if(value == null) return null;
		return convertAsBuyCurrAmtStr(BigDecimal.valueOf(value));
	}
	
	private String convertAsBuyCurrAmtStr(BigDecimal value) {
		if(value == null) return null;
		BigDecimal v = quoteItem.convertToBuyValue(value);
		return v==null ? null : df5.format(v);
	}
	
	private String convertAsRfqCurrAmtStr(Double value) {
		if(value == null) return null;
		return convertAsRfqCurrAmtStr(BigDecimal.valueOf(value));
	}

	private String convertAsRfqCurrAmtStr(BigDecimal value) {
		if(value == null) return null;
		BigDecimal v = quoteItem.convertToRfqValue(value);
		return v==null ? null : df5.format(v);
	}
	
	private String calExRate() {
		BigDecimal exRate = this.quoteItem.getExRate();
		if(exRate != null) {
			return df5.format(exRate);
		}
		return null;
	}
	...
	public void setBuyCurrStr(String buyCurrStr) {
		if (hasDifferentValue(null, this.buyCurrStr, buyCurrStr))
			this.changed = true;
	...
	public void setTargetResaleStr_buy(String buyTargetResaleStr) {
		this.targetResaleStr_buy = buyTargetResaleStr;
	}

	public String getExRateBuyStr() {
		return exRateBuyStr;
	}

	public void setExRateBuyStr(String exRateBuyStr) {
		this.exRateBuyStr = exRateBuyStr;
	}

	public String getExRateRfqStr() {
		return exRateRfqStr;
	}

	public void setExRateRfqStr(String exRateRfqStr) {
		this.exRateRfqStr = exRateRfqStr;
	}
	...
	public PricerInfo createPricerInfo() {
    	...
    	pricerInfo.setRfqCurr(getQuoteItem().getRfqCurr().toString());
    	pricerInfo.setSubmitDate(getQuoteItem().getSubmissionDate());
    	pricerInfo.setExRateDate(getQuoteItem().getExRateDate());
		return pricerInfo;
	}
	...
	public void fillBackQuoteItem() {
	    ...
	    quoteItem.setBuyCurr(Currency.valueOf(buyCurrStr));
	    // add for holding sc type;
	    quoteItem.setSalesCostType(SalesCostType.GetSCTypeByStr(salesCostTypeName));
	    //(Currency.valueOf(buyCurrStr));
	}
