...
//	ICacheUtil cacheUtil;
    ...
    public void saveQuoteItemsInInternalTransfer(List<QuoteItemVo> quoteItemVos, List<Attachment> removedAttachments, User user){
    	...
    	for(QuoteItemVo vo : quoteItemVos){
    		...
    		if("-SELECT-".equalsIgnoreCase(item.getCostIndicator())){
...
//    		item.setInternalTransferComment(vo.getAqccStr());
    	...
    	try {
	    	...
	    	//throw new WebQuoteRuntimeException("Exception Occured while updating cost indicator in sendInternalTransferToQC Method : "+e.getMessage());
