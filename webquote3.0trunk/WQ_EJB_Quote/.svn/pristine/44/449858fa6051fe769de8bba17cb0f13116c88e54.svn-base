...
//	private String aqccStr;
	...
	public Double getFinalQuotationPriceForRevise() {
		if (this.quoteItem.getFinalQuotationPrice()==null) {
			if (this.quoteItem.getQuotedPrice()!=null)
				this.finalQuotationPriceForRevise = this.quoteItem.convertToRfqValueWithDouble(this.quoteItem.getQuotedPrice()).doubleValue(); //this.quoteItem.getQuotedPriceRFQCur();
			else 
				this.finalQuotationPriceForRevise = null;
		}
...
//			when the customer name of quote item is not empty, then show it, otherwise we need to extract the customer name from its master table then show master table��s name
	...
	*/  
	...
	public double getQuotedPriceNewExRate() {
		...
		if (getExRateCalc()!=null && quoteItem.getQuotedPrice()!=null){
