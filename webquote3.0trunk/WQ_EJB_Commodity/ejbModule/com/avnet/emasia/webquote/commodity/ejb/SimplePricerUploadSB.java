...
public class SimplePricerUploadSB  extends PricerUploadCommonSB {
	...
	//
	public Pricer getPricerByComponentKey(PricerUploadTemplateBean bean) {
		...
		if(bean.getOcostIndicator() !=null) {
			sql.append(" AND p.costIndicator = :costIndicator ");
		}
		...
		if(bean.getOcostIndicator() !=null) {
			query.setParameter("costIndicator", bean.getOcostIndicator());
		}
		if(!QuoteUtil.isEmpty(qedStringInFile)) {
	...
	public List<Pricer> getAllOverLapSimplePricers(PricerUploadTemplateBean bean, Long pricerId) {
		Material material = bean.getMaterial();
		if(material == null ||bean == null ){
	...
	public void insertUploadOperater(List<PricerUploadTemplateBean> items, String materialTypeStr,User user, List<Manufacturer> manufacturerLst, int[] countArr, PricerUploadParametersBean puBean)  {
		...
		for (PricerUploadTemplateBean bean : items) {
			bean.setPricer(this.getPricerByComponentKey(bean));
