...
DECLARE
  ...
  cDROP_TBL_DATE CONSTANT VARCHAR2(10 BYTE) := '161205';  -- Date where the table is backup
  cCOMMENT CONSTANT VARCHAR2(100 BYTE) := 'Damon: Backup for adding Description column to SAP_MATERIAL ';  -- Comment to add on backup tables
 ...
 arrBuf := vCharArrayType('SAP_MATERIAL', 'SCHEDULE_CONFIG' );
-- Input respective backup table name in same order as "arrBuf"
arrBufNew := vCharArrayType('SAP_MATERIAL', 'SCHEDULE_CONFIG' );
    recCnt := 0;
