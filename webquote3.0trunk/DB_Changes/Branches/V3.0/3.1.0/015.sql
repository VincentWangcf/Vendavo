...
Insert into role (ID,CREATED_ON,LAST_UPDATED_ON,VERSION,NAME,CREATED_BY,LAST_UDPATED_BY,ACTIVE,DESCRIPTION) 
values ((select max(id)+ 1 from role),sysdate,sysdate,4,'ROLE_CENTRAL_PRICING',(select t.id from app_user t where t.employee_id='901518'),(select t.id from app_user t where t.employee_id='901518'),1,'Central Pricing Team');
