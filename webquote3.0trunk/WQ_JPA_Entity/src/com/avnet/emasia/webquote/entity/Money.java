...
public class Money implements Comparable<Money> {
	...
	public static Money of(Double amount, Currency curr) {
		...
		try {
			return new Money(BigDecimal.valueOf(amount), curr);
		} catch (Exception e) {
			return null;
		}
	}
	...
	public static Money of(Double amount, String curr) {
		...
		try {
			return new Money(BigDecimal.valueOf(amount), Currency.valueOf(curr));
		} catch (Exception e) {
			return null;
		}
	}
	...
	private Money convertTo(Currency toCurrency, BizUnit bizUnit, Customer soldToCustomer, Date date, boolean toRfq, int ... scale) {
		...
		if(toCurrency == this.currency) {
			Money.of(BigDecimal.valueOf(this.amount.doubleValue()), this.currency);
		}
		...
		if (rates[0] == null || rates[1] == null) {
			...
			BigDecimal valueTo = calculatorAsToCurrencyAmount(valueFrom, exFrom, exTo, handling, toRfq, scale);
			return Money.of(valueTo, toCurrency);
		}
	}
	
	private static BigDecimal calculatorAsToCurrencyAmount(BigDecimal valueFrom, BigDecimal exFrom, BigDecimal exTo, BigDecimal handling, boolean toRfq, int ... scale) {
	...
	private static BigDecimal calculatorAsToCurrencyAmountForDlink(BigDecimal valueFrom, BigDecimal exFrom, BigDecimal exTo, Integer scale, Integer roundType) {
	...
	public Money convertToForDlink(Currency toCurrency, BizUnit bizUnit, Customer soldToCustomer, Date date) {
		...
		if (rates[0] == null || rates[1] == null) {
			...
			BigDecimal exFrom = rates[0].getExRateTo();
			BigDecimal valueFrom = this.getAmount();
			BigDecimal valueTo = calculatorAsToCurrencyAmountForDlink(valueFrom, exFrom, exTo, rates[1].getScale(), rates[1].getRoundType()); 
			return Money.of(valueTo, toCurrency);
		}
	}
	/***
	...
	public static ExchangeRate[] findExchangeRate(Currency fromCurr, Currency toCurr, BizUnit bizUnit, Customer soldToCustomer,
		...
		if (toCurr == Currency.USD && fromCurr == toCurr) {
			return new ExchangeRate[] {ExchangeRate.getExchangeRateOne().clone(), ExchangeRate.getExchangeRateOne().clone()};	
		}
		...
		if (exchRateFrom == null ^ exchRateTo == null) { // if either exchRateFrom or exchRateTo is null, no need check customer  
		...
		// use deepCopy to avoid influence these cached ExchangeRate Object.
		if(exchRateFrom != null) exchRateFrom = exchRateFrom.clone();
		if(exchRateTo != null) exchRateTo = exchRateTo.clone();
		
		if (fromCurr == toCurr) {
			// set two ExchangeRates' handling = 1, let no cost when
			// transform between the same currency.
			if(exchRateFrom != null) exchRateFrom.setHandling(BigDecimal.ONE);
			if(exchRateTo != null) exchRateTo.setHandling(BigDecimal.ONE);
		}
	...
	private static List<ExchangeRate> findExchangeRateFromCache(Currency toCurrency, BizUnit bizUnit, 
		...
		if(toCurrency == null || bizUnit == null || date == null || exchangeRates == null) 
		{
			/*logger.severe(MessageFormat.format("findExchangeRateFromCache meet null pagram data . toCurrency:[{0}],bizUnit:[{1}], date:[{2}],handling:[{3}], exchangeRates counts:[{3}] ", 
					toCurrency, bizUnit, date, exchangeRates.size()));*/
			 throw new NullPointerException(MessageFormat.format("findExchangeRateFromCache meet null parameter data . toCurrency:[{0}],bizUnit:[{1}], date:[{2}],handling:[{3}], exchangeRates counts:[{3}] ", 
						toCurrency, bizUnit, date, exchangeRates.size()));
		}
	 ...
	 * @return
	 */
	public static ExchangeRate[] getExchangeRate(Currency fromCurr, Currency toCurr, BizUnit bizUnit, 
			Customer soldToCustomer, Date date) {
		List<ExchangeRate> exchangeRates = cacheUtil.getExchangeRates();
		return findExchangeRateStrategy(fromCurr, toCurr, bizUnit, soldToCustomer, date, exchangeRates);
	}
	
	//see above.
	public static ExchangeRate[] getExchangeRate(String fromCurr, String toCurr, String bizUnit, 
			Customer soldToCustomer, Date date) {
		 
		if(fromCurr==null || toCurr==null || bizUnit==null || date ==null) {
			logger.severe(MessageFormat.format("getExchangeRate(String,String....) meet null parameters data. "
					+ "fromCurr:[{0}], toCurr:[{1}], bizUnit:[{2}], date:[{3}] ", 
					fromCurr, toCurr, bizUnit, date));
			throw new NullPointerException();
			//logger.log(Level.SEVERE, "", new NullPointerException());
			//empty and size of 2 .
			 //return new ExchangeRate[2];
		 }
	 ...
	 * 'value of rfq of buy currency' , use 'isFromCurrToRfqCurr' param to indicator convert path.
	 * @param valueFrom
	 * @param exRateFrom
	 * @param exRateTo
	 * @param handling
	 * @param isFromCurrToRfqCurr
	 * @return
	 */
	public static BigDecimal convertAsToCurrAmtWithExrates(BigDecimal valueFrom, BigDecimal exRateFrom, BigDecimal exRateTo, 
			BigDecimal handling, boolean isFromCurrToRfqCurr, int ... scale) {
		if(valueFrom == null || exRateFrom == null || exRateTo == null || handling == null) {
			logger.info(() -> MessageFormat.format("valueFrom:[{0}],exRateFrom:[{1}], exRateTo:[{2}],handling:[{3}] ", 
					valueFrom, exRateFrom, exRateTo, handling));
			return null;
		}
		//new BigDecimal("0.0").equals(BigDecimal.ZERO) false;
		//new BigDecimal("0.000").compareTo(BigDecimal.ZERO)==0 true;
		//using compareTo match we want .
		if(exRateFrom.compareTo(BigDecimal.ZERO) == 0) {
			logger.severe("Can not use 0 to divide , handle it as null");
			return null;
		}
		return calculatorAsToCurrencyAmount(valueFrom, exRateFrom, exRateTo, handling, isFromCurrToRfqCurr, scale);
	}
	
	//see above
	public static Double convertAsToCurrAmtWithExrates(Double valueFrom, BigDecimal exRateFrom, BigDecimal exRateTo, BigDecimal handling,
			boolean isFromCurrToRfqCurr, int ... scale) {
		if(valueFrom == null)  {
			logger.info(() -> MessageFormat.format("valueFrom:[{0}]", valueFrom));
			return null;
		}
		BigDecimal v = convertAsToCurrAmtWithExrates(BigDecimal.valueOf(valueFrom), exRateFrom, exRateTo, handling, isFromCurrToRfqCurr, scale);
	 ...
	 * @return
	 */
	public static List<QuantityBreakPrice> convertAsToCurrAmtWithExrates(List<QuantityBreakPrice> qts, BigDecimal exRateFrom, BigDecimal exRateTo, BigDecimal handling, boolean isFromCurrToRfqCurr) {
		if(qts == null || qts.isEmpty()) {
			logger.warning("qts is null or empty");
			return qts;
		}
		
		qts.stream().forEach(q -> {
			q.setQuantityBreakPrice(convertAsToCurrAmtWithExrates(q.getQuantityBreakPrice(), exRateFrom, exRateTo, handling, isFromCurrToRfqCurr));
			q.setSalesCost(convertAsToCurrAmtWithExrates(q.getSalesCost(), exRateFrom, exRateTo, handling, isFromCurrToRfqCurr));
			q.setSuggestedResale(convertAsToCurrAmtWithExrates(q.getSuggestedResale(), exRateFrom, exRateTo, handling, isFromCurrToRfqCurr));
		});
	 ...
	 * @return
	 */
	public static List<QuantityBreakPrice> convertAsToCurrAmtWithExrates(List<QuantityBreakPrice> qts, Currency fromCurr, Currency toCurr, BizUnit bizUnit, Customer soldToCustomer, Date date, Currency rfqCurrency) {
		if(fromCurr == toCurr) {
			return qts;
		}
		if(qts == null || qts.isEmpty()) {
		...
		if(fromCurr==null || toCurr==null || bizUnit==null || date ==null) {
			 logger.warning(() -> MessageFormat.format("convertAsToCurrAmtWithExrates, fromCurr[{0}],toCurr[{1}],bizUnit[{2}],date[{3}]", fromCurr, toCurr, bizUnit, date));
			 return null;
		}
		...
		if(rates == null || rates.length != 2 || Arrays.stream(rates).anyMatch(r -> null == r)) {
			logger.severe("The rates are " + Arrays.toString(rates));
		}
		...
		return convertAsToCurrAmtWithExrates(qts, exFrom, exTo, handling, toCurr.equals(rfqCurrency));
	}
	 ...
	 * @return
	 */
	public static Double convertAsToCurrAmt(Double valueFrom, Currency fromCurr, Currency toCurr, BizUnit bizUnit, Customer soldToCustomer, Date date, Currency rfqCurr) {
		if(fromCurr == toCurr || valueFrom == null) {
			return valueFrom;
		}
		if(fromCurr==null || toCurr==null || bizUnit==null || date ==null) {
			 logger.warning(() -> MessageFormat.format("convertAsToCurrAmt, fromCurr[{0}],toCurr[{1}],bizUnit[{2}],date[{3}]",
					 fromCurr, toCurr, bizUnit, date));
			 return null;
		}
