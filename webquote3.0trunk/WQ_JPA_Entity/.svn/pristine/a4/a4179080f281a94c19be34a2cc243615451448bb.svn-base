...
public class PageComponentItem {
	...
	@ManyToOne
	@JoinColumn(name = "LOGIC_CAL_CONFIG_ID", nullable = false)
	private LogicCalConfig logicCalConfig;
	
	public LogicCalConfig getLogicCalConfig() {
		return logicCalConfig;
	}

	public void setLogicCalConfig(LogicCalConfig logicCalConfig) {
		this.logicCalConfig = logicCalConfig;
	}
	...
	public boolean isConditionMatch(List<String> regions, List<String> roles) {
		...
		return this.logicCalConfig.isRegionAndRoleMatched(rgs, rs).test(true);
	}*/
	
	@SuppressWarnings("unchecked")
	public boolean isConditionMatch(EnumMap<ConditionType,Collection<String>> mapParam) {
		return this.logicCalConfig.isMatched(mapParam).test(true);
	}
